...
const OrderDetail = () => {
  ...
  return (
    <div style={{background:'white',boxShadow:'var(--box-shadow)',borderRadius:'20px',padding:'2px'}}>
      {loading ? (
      ...
      <div>
        <div className='class_justify_contents_row withGap' style={{flexDirection:window.innerWidth<760&&'column',justifyContent:'space-around'}}>
      <div>
<div style={{cursor:'pointer'}} onClick={()=>navigate(-1)}><BiArrowBack size={'40px'}/></div>
      <h3>Order:{order.order_id}</h3>
        <h3>Ordered On:{moment(order.createdAt).format("MM/DD/YYYY hh:mm A")}</h3>
      </div>
        ...
        <div style={{padding:'2rem'}}>
          <h3>Items:</h3>
          {order.products.map((product)=><Items item={product} />)}
        </div>
      </div>}
